...
class Purchase:
    def __init__(self):
        ...
        lb7 = Label(self.root, text="Costumer Name")
    ...
    def generatebillno(self):
        try:
            ...
            with self.pymysqlobj.cursor() as myconnectionobj:
                query = "select max(billno) from purchasestable"
                try:
    ...
    def saveinfo(self):
        ...
        try:
            with connectionobj.cursor() as myconnection:
                query = "insert into purchasestable values(%s, %s,%s,%s)"
                try:
                    ...
                    for content in allrows:
                        ...
                        query = "insert into purchasedetail values(%s,%s,%s,%s,%s,%s)"
                        try:
                            myconnection.execute(query, (self.billnobox.cget("text"),
                                                         pid,
                                                         pname, price, qty, tc
                            ...
                            connectionobj.commit()
                            query = "update categorytable set qty=qty+%s where pid=%s"
                            try:
